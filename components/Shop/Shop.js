class Shop {
  ...
  productsStore = localStorageUtil.getProducts();
  render() {
    let htmlCatalog = '';
    let htmlCatalog2 = '';
    let summCatalog = 0;
    this.productsStore.forEach((item) => {
      const cartItem = this.catalog.find(cartEl => Number(cartEl.id) === Number(item.id))
        htmlCatalog += `
      ...
      <button class='shop-btn__plus' onclick="shopPage.increaseCount(${id})">+</button>
      </div>
      </li>
        `;
        summCatalog += price;
