class Shop {
  ...
  render() {
    ...
    let htmlCatalog2 = '';
    let summCatalog = 0;
    CATALOG.forEach(({id, count, name, price, img}) => {
      if(productsStore.indexOf(id) !== -1) {
        ...
        <button class='shop-delete' onclick="shopPage.deleteItems(this, '${id}')"><svg width="21" height="17" viewBox="0 0 21 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div>
      <button class='shop-btn__minus' onclick="shopPage.decreaseCount(${id})">-</button>
      <span class='shop-value__counter'>${count}</span>
      <button class='shop-plus' onclick="shopPage.increaseCount(${id})">+</button>
      </div>
      </li>
        `;
        summCatalog += price;
